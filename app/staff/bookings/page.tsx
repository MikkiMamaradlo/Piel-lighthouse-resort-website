  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-slate-800 dark:text-white">Bookings Management</h1>
          <p className="text-slate-500 dark:text-slate-300 mt-1">Manage and track all guest reservations</p>
        </div>
        <div className="flex flex-col sm:flex-row gap-4">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="pl-10 pr-4 py-2.5 w-full sm:w-64 border border-slate-200 dark:border-slate-600 rounded-xl focus:ring-2 focus:ring-amber-500 focus:border-amber-500 outline-none transition-all bg-white dark:bg-slate-800 text-slate-800 dark:text-white placeholder-slate-400 dark:placeholder-slate-500"
            />
          </div>
        </div>
      </div>
          ...
          <button
            ...
            className={`group relative px-5 py-2.5 rounded-xl font-medium text-sm transition-all duration-200 ${
              filter === tab.key
                ...
                : "bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:bg-slate-700/50 border border-slate-200 dark:border-slate-600"
            }`}
          >
            <span className="flex items-center gap-2">
              {tab.label}
              <span className={`px-2 py-0.5 rounded-full text-xs ${
                filter === tab.key ? "bg-white dark:bg-slate-800/20" : "bg-slate-100 dark:bg-slate-700"
              }`}>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 overflow-hidden">
        <div className="p-6 border-b border-slate-100 dark:border-slate-700 flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-semibold text-slate-800 dark:text-white">All Bookings</h2>
            <p className="text-sm text-slate-500 dark:text-slate-300">{filteredBookings.length} bookings found</p>
          </div>
        </div>
          ...
          <table className="w-full">
            <thead className="bg-slate-50 dark:bg-slate-700/50">
              <tr>
                ...
                <th className="px-6 py-4 text-left text-xs font-semibold text-slate-500 dark:text-slate-300 uppercase tracking-wider">Room</th>
                <th className="px-6 py-4 text-left text-xs font-semibold text-slate-500 dark:text-slate-300 uppercase tracking-wider">Dates</th>
                <th className="px-6 py-4 text-left text-xs font-semibold text-slate-500 dark:text-slate-300 uppercase tracking-wider">Guests</th>
                ...
                <th className="px-6 py-4 text-left text-xs font-semibold text-slate-500 dark:text-slate-300 uppercase tracking-wider">Booked</th>
              </tr>
            </thead>
                ...
                >
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="text-sm font-medium text-slate-800 dark:text-white">{booking.guestName}</div>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4">
                    <span className="text-sm font-medium text-slate-800 dark:text-white">{booking.roomType}</span>
                  </td>
                  <td className="px-6 py-4 text-sm">
                    <div className="flex items-center gap-2">
                      <div className="text-slate-600 dark:text-slate-300">{formatDate(booking.checkIn)}</div>
                      <svg className="w-4 h-4 text-slate-300 dark:text-slate-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 8l4 4m0 0l-4 4m4-4H3" />
                      </svg>
                      <div className="text-slate-600 dark:text-slate-300">{formatDate(booking.checkOut)}</div>
                    </div>
                  </td>
                  <td className="px-6 py-4">
        ...
        {filteredBookings.length === 0 && (
          <div className="p-12 text-center">
            <div className="w-16 h-16 bg-slate-100 dark:bg-slate-700 rounded-full flex items-center justify-center mx-auto mb-4">
              <BookingIcon className="w-8 h-8 text-slate-400 dark:text-slate-300" />
            </div>
            <div className="text-slate-500 dark:text-slate-300">No bookings found</div>
            {(filter !== "all" || search) && (
              <button
